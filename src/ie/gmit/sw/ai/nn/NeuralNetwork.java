...
public class NeuralNetwork{
	...
	private double[][] inputHidW; //Matrix of weights for input->hidden layer
	private double[][] outputHidW; //Matrix of weights for hidden->output layer
	...
	public NeuralNetwork(Activator.ActivationFunction function, int num_input_nodes, int num_hidden_nodes, int num_output_nodes) {
		...
		this.inputHidW = new double[num_input_nodes + 1][num_hidden_nodes]; //An extra row for the bias
		this.outputHidW = new double[num_hidden_nodes + 1][num_output_nodes]; //An extra row for the bias
		
		this.initialiseWeights(inputHidW);
		this.initialiseWeights(outputHidW);
	}
	...
	public void feedForward(){ 
		...
		for (int hid = 0; hid < hidden.length; hid++){
			sum = 0.0d;
			for (int in = 0; in < inputs.length; in++) sum += inputs[in] * inputHidW[in][hid];
			sum+= inputHidW[inputs.length][hid];
		...
		for (int out = 0; out < outputs.length; out++){
			sum = 0.0d;
			for (int hid = 0; hid < hidden.length; hid++)	sum += hidden[hid] * outputHidW[hid][out];		
			sum+= outputHidW[hidden.length][out];
